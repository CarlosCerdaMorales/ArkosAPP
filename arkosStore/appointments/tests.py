...
from appointments.models import Appointment, Service, Availability, StatusChoices
...
class AppointmentLogicTest(TestCase):
    ...
    def setUp(self):
        ...
        self.service_60 = Service.objects.create(
            name=TypeChoices.OSTEOPATHY_MASSAGE, 
            duration=60
        )

        self.service_30 = Service.objects.create(
            name=TypeChoices.NUTRITIONAL_ADVICE, 
            duration=30
        )
...
class AppointmentViewsTest(TestCase):
    ...
    def setUp(self):
        ...
        """
        self.client = Client()
        self.user = User.objects.create_user(username='testuser', password='password', email='test@example.com', phone_number='+34 600111222')
        self.other_user = User.objects.create_user(username='other', password='password', email='other@example.com', phone_number='+34 600333444')
        
        self.worker = Worker.objects.create(name='Worker Test')
        self.service = Service.objects.create(name=TypeChoices.OSTEOPATHY_MASSAGE, duration=60)
    ...
    def test_history_view_logic(self):
        ...
        app1 = Appointment.objects.create(user=self.user, worker=self.worker, service=self.service, datetime=past_date, status=StatusChoices.COMPLETED)
        
        Appointment.objects.create(user=self.user, worker=self.worker, service=self.service, datetime=timezone.now() + timedelta(days=1), status=StatusChoices.CONFIRMED)
        
        Appointment.objects.create(user=self.other_user, worker=self.worker, service=self.service, datetime=past_date, status=StatusChoices.COMPLETED)
    ...
    def test_upcoming_view_logic(self):
        ...
        app1 = Appointment.objects.create(user=self.user, worker=self.worker, service=self.service, datetime=future_date, status=StatusChoices.CONFIRMED)
        
        Appointment.objects.create(user=self.user, worker=self.worker, service=self.service, datetime=timezone.now() - timedelta(days=1), status=StatusChoices.COMPLETED)
    ...
    def test_create_appointment_success(self):
        ...
        self.assertTrue(Appointment.objects.filter(user=self.user, datetime__hour=10).exists())
    ...
    def test_cancel_valid(self):
        ...
        """
        future_date = timezone.now() + timedelta(hours=24)
        app = Appointment.objects.create(user=self.user, worker=self.worker, service=self.service, datetime=future_date)
    ...
    def test_cancel_invalid_too_late(self):
        ...
        """
        soon_date = timezone.now() + timedelta(hours=2)
        app = Appointment.objects.create(user=self.user, worker=self.worker, service=self.service, datetime=soon_date)
    ...
    def test_cancel_other_user_appointment(self):
        ...
        """
        future_date = timezone.now() + timedelta(hours=24)
        app_other = Appointment.objects.create(user=self.other_user, worker=self.worker, service=self.service, datetime=future_date)
    ...
    def test_api_get_slots(self):
        ...
        response = self.client.get(url, {'service_id': self.service.id, 'date': next_monday})
